...
| 📱 **发消息** | 手机向**任意 CC 会话**发送指令（`claude -p --resume` 续聊执行），结果实时回显；电脑占用该会话时自动排队、切走后自动执行，被打断自动重试；支持 `/clear`、`/skills`、`/model` 等命令与技能调用 |
...
一个会话同一时间只属于一端 —— 这是由 Claude Code 会话文件的特性决定的（两个实例并发写会让消息**穿插进同一个上下文**，互相干扰）。本服务自动维护这条规则：

1. **电脑占用时手机排队**：电脑 CC 停留在某会话（无论是否在生成）时，手机发给该会话的消息进入队列，电脑切走后**自动执行**；检测基于 CC 自带的会话登记文件（`~/.claude/sessions/`），即时生效
2. **手机任务被打断自动重试**：手机任务执行中若电脑端接管该会话，任务自动暂停并重新排队（最多重试 2 次），避免与电脑消息交错污染上下文
3. 手机端可随时取消：任务进行中按钮显示「停止」，排队中显示「取消排队」
4. 电脑想看手机的操作结果：监视页实时可见，或在 CC 中用 `claude --resume <会话ID>` 重开即读回完整历史
5. 想要真并行请各开各的会话
