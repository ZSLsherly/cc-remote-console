...
| 📱 **发消息** | 手机向**任意 CC 会话**发送指令（`claude -p --resume` 续聊执行），结果实时回显；**电脑优先**：电脑执行命令时手机发送被拒、手机任务立即终止；手机可强制停止自己的任务，也能**优雅打断电脑端任务**（Ctrl+C 语义，不关窗口，网页终端内的 CC 有效）；支持 `/clear`、`/skills`、`/model` 等命令与技能调用 |
| 🖥️ **终端** | 完整 bash 终端（Git Bash 登录 shell，自带 DeepSeek 环境可直接跑 `claude`）；多终端标签：切换 / 关闭 / 重命名 / 断线重连回放 3000 行；⏹ 打断按钮（Ctrl+C，手机键盘没有 Ctrl 也能中断命令） |
...
一个会话同一时间只属于一端 —— 这是由 Claude Code 会话文件的特性决定的（两个实例并发写会让消息**穿插进同一个上下文**，互相干扰）。本服务按「电脑优先」维护这条规则：

1. **电脑正在执行命令时**（生成回复中）：手机发送被拒绝（423，稍后再发）；电脑只是**停在**会话里（未在输入）不影响手机操作
2. **手机任务执行中，电脑一开始执行命令**（提交输入）：手机任务被立即终止（2 秒内），手机端显示"任务已被电脑终止"，**不排队、不自动重试**；电脑只是在输入框里打字不会触发
3. **手机强制停止**：任务执行中发送键变为「⏹ 停止」，强杀进程树，随时可在外面中断并改发新指令
4. **手机打断电脑端**：电脑 CC 在该会话运行时，发送键变为「⏹ 终止电脑端」，向电脑端 CC 发送 **Ctrl+C（等同电脑上按一次，只打断当前任务、不关闭窗口）**。受 Windows 限制，只有跑在网页「终端」标签页里的电脑端 CC 能被外部投递 Ctrl+C（按进程树祖先判断）；跑在 Windows Terminal 等外部终端里的 CC 无法注入，手机会如实提示 —— 建议电脑端 CC 直接用网页终端开（`终端`标签页里跑 `claude`），手机即可随时优雅打断
5. **终端打断**：终端标签页有「⏹ 打断」按钮（发送 Ctrl+C），手机键盘没有 Ctrl 键也能中断终端里正在跑的命令（包括终端里的 claude）；「✕」则直接关闭整个终端
6. 电脑想看手机的操作结果：监视页实时可见，或在 CC 中用 `claude --resume <会话ID>` 重开即读回完整历史
7. 想要真并行请各开各的会话
