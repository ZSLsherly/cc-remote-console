...
| 📱 **发消息** | 手机向**任意 CC 会话**发送指令（`claude -p --resume` 续聊执行），结果实时回显；**电脑优先**：电脑执行命令时手机发送被拒、手机任务立即终止；手机可强制停止自己的任务，也能**强制终止电脑端 CC**（Ctrl+C → 强杀）接管会话；支持 `/clear`、`/skills`、`/model` 等命令与技能调用 |
...
4. **手机终止电脑端**：电脑 CC 在该会话运行（无论是否在生成）时，发送键变为「⏹ 终止电脑端」——先尝试向电脑 CC 的控制台发 Ctrl+C 优雅中断，2.5 秒内未退出则强杀进程树（关闭电脑端 CC 窗口）。会话内容完整保留在磁盘，电脑端 `claude --resume` 重开即可恢复；该会话随即释放，手机可立即接管
