...
| 📱 **发消息** | 手机向**任意 CC 会话**发送指令（`claude -p --resume` 续聊执行），结果实时回显；**电脑优先**：电脑执行命令时手机发送被拒、手机任务立即终止；手机可强制停止；支持 `/clear`、`/skills`、`/model` 等命令与技能调用 |
...
4. **终端打断**：终端标签页有「⏹ 打断」按钮（发送 Ctrl+C），手机键盘没有 Ctrl 键也能中断终端里正在跑的命令（包括终端里的 claude）；「✕」则直接关闭整个终端
5. 电脑想看手机的操作结果：监视页实时可见，或在 CC 中用 `claude --resume <会话ID>` 重开即读回完整历史
6. 想要真并行请各开各的会话
