...
| 📱 **发消息** | 手机向**任意 CC 会话**发送指令（`claude -p --resume` 续聊执行），结果实时回显；正在运行中的会话自动拦截（防并发冲突） |
...
- 手机发消息与电脑终端**操作的是同一份会话文件**，遵循：
  1. 会话在电脑上**正在运行**时，手机发送会被拒绝（423，运行中检测窗口 60 秒）
  2. 电脑会话关闭后，手机可接管任意会话；电脑想看手机的操作结果：监视页实时可见，或在 CC 中用 `claude --resume <会话ID>` 重开即读回完整历史
  3. 同一会话同一时间只在一个端输入；想要真并行请各开各的会话
- 已知限制：电脑端执行超 60 秒无输出的长任务时，"运行中"标记可能暂时失效，注意勿在此时用手机发同一会话
